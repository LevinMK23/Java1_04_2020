...
public class HW {
    ...
    private static boolean checkVictory(char[][] map, char dot) {
        boolean rightD = true, leftD = true;
        int size = map.length;
        for (int i = 0; i < size; i++) {
    ...
    public static void game() {
        ...
        while (true) {
            ...
            if (isCellValid(map, x, y)) {
                ...
                System.out.println("Введены некорректные данные! Либо ячейка занята либо номер строки и столбца не попадают в диапозоне: от 1 до " + map.length);
            }
        }
