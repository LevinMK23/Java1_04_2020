...
public class GameXO extends JFrame {
    ...
    private File soundWin = new File("src\\res\\WinSound.wav");
    private File soundLose = new File("src\\res\\LoseSound.wav");
    ...
    private void checkGame() throws UnsupportedAudioFileException, IOException, LineUnavailableException {
        ...
        if (this.checkWin(this.DOT_X_IMG, this.map)) {
            ...
            JOptionPane.showMessageDialog(this, "С победой тебя, Дружище!", "Игра закончена! Вы выиграли.", -1);
            this.newGame();
        } else if (this.isMapFull()) {
            JOptionPane.showMessageDialog(this, "Ничья", "Игра закончена! Ничья.", -1);
            this.newGame();
        } else {
            this.aiTurn();
            if (this.checkWin(this.DOT_O_IMG, this.map)) {
                this.playSoundOnClick(this.soundLose);
                JOptionPane.showMessageDialog(this, "Компьютер оказался умней(", "Игра закончена! Вы проиграли.", -1);
                this.newGame();
            } else if (this.isMapFull()) {
                JOptionPane.showMessageDialog(this, "Ничья", "Игра закончена! Ничья.", -1);
                this.newGame();
